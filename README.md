...
- **Анимация нижней капсулы** (`native.scss`): всплытие при запуске, скользящий «пузырь» под активной
  вкладкой (позиция — по `li.active` через `:has()`, пружинный перелёт), «подпрыгивание» иконки,
  сжатие при нажатии; при `prefers-reduced-motion` отключается. Колонки вкладок принудительно равные
