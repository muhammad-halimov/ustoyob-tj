...
### Пагинация (грабли: «показывает 25 из 32»)
- Коллекции API постраничные: **по умолчанию 25, максимум 50**, а завышенный `itemsPerPage` (500, 100…) бэкенд **молча режет до 50**. Запрос без пагинации видит только первую страницу.
- Где нужен **весь** набор без «Показать ещё» (справочники, избранное, чёрный список, все чаты, свои обращения в ТП, объявления в формах отзыва/жалобы, счётчик непрочитанных) — используйте `fetchAllPages(endpoint, opts)` из `utils/paginationUtils.ts`: он сам ставит `itemsPerPage=50`, добирает страницы (по `totalItems`, а для голого массива — пока приходят полные страницы) и считает 404 пустым списком. Справочники (`getCategories`, `getOccupations`, `getDistricts`…) через `dataCacheUtils` уже это делают.
- Списки с «Показать ещё» (профиль, категория, чаты, отзывы, избранное, «Мои объявления») страницы запрашивают сами (`page`/`itemsPerPage` + `parsePagedResponse`); размер страницы даёт `getPageSize()`, он ограничен 50.
- Приложение шлёт `Accept: application/json` → приходит **голый массив без `totalItems`**; в `application/ld+json` ключи `member`/`totalItems` (без `hydra:`) — не путайте при ручных `curl`.
