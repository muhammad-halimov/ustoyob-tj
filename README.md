...
- **Телефоны пользователей** (`phones`) видит только авторизованный, и только в карточке: `GET /users/me`, `GET /users/{id}`, ответ `PATCH`. В списках (`GET /users`) и во вложенных пользователях (мастер/автор в тикетах, участники чатов, авторы отзывов) их **нет** — чтобы номера нельзя было собрать пагинацией. Логика — `src/Serializer/UserContextBuilder.php`; фронт запрашивает `GET /users/{id}`, когда нужно показать номер.
