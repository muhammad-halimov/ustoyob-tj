...
- **Специалист** → оставляет отзыв заказчику (type: 'client')
- **Заказчик** → оставляет отзыв специалисту (type: 'master')
...
### С обработкой успеха через Status
...
Компонент использует CSS модули (`Review.module.scss`):
