 ...
 *
 * ⚠️ Это НЕ HTML-санитайзер — regex-стрип тегов ловит обычную разметку, но не
 * гарантирует безопасность произвольного HTML (edge-кейсы вроде атрибутов с `>`
 * внутри кавычек). Результат безопасен только как обычный текстовый JSX-узел
 * (React сам экранирует при рендере) — НИКОГДА не передавайте его в
 * dangerouslySetInnerHTML. Для рендера доверенного HTML с санитайзингом
 * используется DOMPurify (см. Legal.tsx) — используйте его, а не эту функцию.
 */
